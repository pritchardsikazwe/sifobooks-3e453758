...
CREATE TABLE IF NOT EXISTS public.hotel_housekeeping_tasks (
  ...
  task_date DATE NOT NULL DEFAULT CURRENT_DATE, task_type TEXT NOT NULL DEFAULT 'cleaning', status TEXT NOT NULL DEFAULT 'pending', assigned_to UUID, notes TEXT, completed_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.hotel_maintenance_tasks (
  ...
  title TEXT NOT NULL, priority TEXT NOT NULL DEFAULT 'medium', status TEXT NOT NULL DEFAULT 'open', description TEXT, estimated_cost NUMERIC(18,2) NOT NULL DEFAULT 0, actual_cost NUMERIC(18,2) NOT NULL DEFAULT 0, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.school_students (
  ...
  date_of_birth DATE, gender TEXT, status TEXT NOT NULL DEFAULT 'active', current_class TEXT, phone TEXT, email TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(company_id, admission_no)
);
CREATE TABLE IF NOT EXISTS public.school_parents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, name TEXT NOT NULL, relationship TEXT, phone TEXT, email TEXT, address TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.school_student_parents (
  ...
  company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE, user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, is_primary BOOLEAN NOT NULL DEFAULT false, PRIMARY KEY(student_id, parent_id)
);
CREATE TABLE IF NOT EXISTS public.school_admissions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, application_no TEXT NOT NULL, applicant_name TEXT NOT NULL, class_applied TEXT NOT NULL, application_date DATE NOT NULL DEFAULT CURRENT_DATE, status TEXT NOT NULL DEFAULT 'new', notes TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(company_id, application_no)
);
CREATE TABLE IF NOT EXISTS public.school_classes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, code TEXT NOT NULL, name TEXT NOT NULL, grade TEXT, stream TEXT, active BOOLEAN NOT NULL DEFAULT true, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(company_id, code)
);
CREATE TABLE IF NOT EXISTS public.school_subjects (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, code TEXT NOT NULL, name TEXT NOT NULL, active BOOLEAN NOT NULL DEFAULT true, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(company_id, code)
);
CREATE TABLE IF NOT EXISTS public.school_attendance (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, student_id UUID NOT NULL REFERENCES public.school_students(id) ON DELETE CASCADE, attendance_date DATE NOT NULL, status TEXT NOT NULL, notes TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(student_id, attendance_date)
);
CREATE TABLE IF NOT EXISTS public.school_assessments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, name TEXT NOT NULL, term TEXT, academic_year TEXT NOT NULL, class_id UUID REFERENCES public.school_classes(id) ON DELETE SET NULL, status TEXT NOT NULL DEFAULT 'draft', max_marks NUMERIC(10,2) NOT NULL DEFAULT 100, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.school_marks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE, user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, assessment_id UUID NOT NULL REFERENCES public.school_assessments(id) ON DELETE CASCADE, student_id UUID NOT NULL REFERENCES public.school_students(id) ON DELETE CASCADE, marks NUMERIC(10,2) NOT NULL DEFAULT 0, grade TEXT, remarks TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(assessment_id, student_id)
);
CREATE TABLE IF NOT EXISTS public.school_fee_accounts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE, user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, student_id UUID NOT NULL REFERENCES public.school_students(id) ON DELETE CASCADE, academic_year TEXT NOT NULL, term TEXT, charges NUMERIC(18,2) NOT NULL DEFAULT 0, paid NUMERIC(18,2) NOT NULL DEFAULT 0, balance NUMERIC(18,2) NOT NULL DEFAULT 0, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(student_id, academic_year, term)
);
...
DO $$ DECLARE t TEXT; BEGIN FOR t IN SELECT unnest(ARRAY['hotel_room_types','hotel_rooms','hotel_guests','hotel_reservations','hotel_folios','hotel_folio_lines','hotel_housekeeping_tasks','hotel_maintenance_tasks','school_students','school_parents','school_admissions','school_classes','school_subjects','school_attendance','school_assessments','school_marks','school_fee_accounts']) LOOP EXECUTE format('DROP POLICY IF EXISTS "own sector records" ON public.%I', t); EXECUTE format('CREATE POLICY "own sector records" ON public.%I FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id)', t); END LOOP;
  DROP POLICY IF EXISTS "own student parent links" ON public.school_student_parents;
  CREATE POLICY "own student parent links" ON public.school_student_parents FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
END $$;
