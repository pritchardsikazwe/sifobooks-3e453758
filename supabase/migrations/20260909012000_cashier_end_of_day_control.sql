...
-- End-of-day is one reconciliation record for a cashier/register/shift/day.
...
CREATE INDEX IF NOT EXISTS pos_end_of_day_user_date_idx
  ON public.pos_end_of_day(user_id, business_date DESC);
CREATE INDEX IF NOT EXISTS pos_end_of_day_location_date_idx
  ON public.pos_end_of_day(location_id, business_date DESC);
...
CREATE INDEX IF NOT EXISTS pos_cash_declarations_eod_idx
  ON public.pos_cash_declarations(end_of_day_id);
...
CREATE POLICY "cash declarations view" ON public.pos_cash_declarations FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.pos_end_of_day e
    WHERE e.id = end_of_day_id
      AND (auth.uid() = e.user_id OR (is_staff_of(e.user_id) AND has_perm('inventory.view', e.user_id)))
  ));
CREATE POLICY "cash declarations insert" ON public.pos_cash_declarations FOR INSERT TO authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.pos_end_of_day e
    WHERE e.id = end_of_day_id
      AND (auth.uid() = e.user_id OR (is_staff_of(e.user_id) AND has_perm('inventory.manage', e.user_id)))
  ));
...
AS $$
  ...
  FROM public.pos_cash_declarations
  WHERE end_of_day_id = _eod;
$$;
...
-- Recalculate the declaration total from denomination rows whenever a declaration changes.
...
BEGIN
  ...
  UPDATE public.pos_end_of_day e
     SET declared_cash = COALESCE((SELECT SUM(d.amount) FROM public.pos_cash_declarations d WHERE d.end_of_day_id = _eod),0),
         updated_at = now()
   WHERE e.id = _eod;
...
CREATE TRIGGER pos_cash_declarations_refresh_eod
AFTER INSERT OR UPDATE OR DELETE ON public.pos_cash_declarations
FOR EACH ROW EXECUTE FUNCTION public.refresh_pos_eod_declared_cash();

-- A reporting view uses ONLY completed POS sales. Held/draft/cancelled sales never enter this total.
...
SELECT
  ...
  COALESCE(SUM(CASE WHEN s.status = 'completed' AND lower(p.method) IN ('mobile_money','momo','airtel_money','mtn_momo') THEN p.amount ELSE 0 END),0) AS mobile_money_sales,
  COALESCE(SUM(CASE WHEN s.status = 'completed' AND lower(p.method) NOT IN ('cash','card','visa','mastercard','bank_card','mobile_money','momo','airtel_money','mtn_momo') THEN p.amount ELSE 0 END),0) AS other_sales,
  e.payouts,
  e.refunds,
  e.status,
  e.expected_cash,
...
-- Seed the existing active MKP register against Chibombo when the location exists.
DO $mkp$
DECLARE _uid uuid := 'e54d7679-cd55-4fee-8811-6b8e260cba75'; _loc uuid;
BEGIN
  SELECT id INTO _loc FROM public.inventory_locations
   WHERE user_id = _uid AND upper(code) = 'CHIBOMBO' AND is_active = true LIMIT 1;
  IF _loc IS NOT NULL THEN
    UPDATE public.pos_registers
       SET location_id = _loc
     WHERE is_active = true
       AND (user_id = _uid OR user_id IS NULL)
       AND (name = 'Register 01' OR branch = 'Main');
  END IF;
END $mkp$;

-- Keep existing shift location consistent with its register where possible.
...
CREATE TRIGGER pos_end_of_day_updated
BEFORE UPDATE ON public.pos_end_of_day
