...
export function useInstalledModules() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (SIFOBOOKS_EDITION === "restaurant") {
      ...
      ["core_home", "sales", "purchases", "inventory", "retail_pos", "hotel_erp", "reports", "admin", "compliance", "learning"].forEach(k => merged.add(k));
    } else if (SIFOBOOKS_EDITION === "school") {
      ["core_home", "sales", "purchases", "inventory", "finance", "school_erp", "hr_payroll", "reports", "admin", "compliance", "learning"].forEach(k => merged.add(k));
