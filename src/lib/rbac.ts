...
export type Access = {
  tenant_id: string; is_owner: boolean; is_super_admin: boolean;
  staff_id?: string | null; full_name?: string | null; role_key?: string | null; role_name?: string | null;
  pos_channel?: "retail" | "restaurant" | null; branch_id?: string | null; branch_name?: string | null;
...
export async function loadAccess(force = false): Promise<Access | null> {
  ...
  inflight = (async () => {
    ...
    if (error || !data) {
      ...
      return { tenant_id: uid, is_owner: true, is_super_admin: false, permissions: [] } as Access;
    }
...
export async function getTenantId(): Promise<string | null> { return (await loadAccess())?.tenant_id ?? null; }
export function hasPerm(a: Access | null | undefined, perm: PermissionKey): boolean {
  if (!a) return false; if (a.is_owner || a.is_super_admin) return true; return a.permissions.includes(perm);
}
export function hasAnyPerm(a: Access | null | undefined, perms: PermissionKey[]): boolean { return perms.some((p) => hasPerm(a, p)); }

type Rule = { prefix: string; any?: PermissionKey[]; ownerOnly?: boolean; open?: boolean };
export const ROUTE_RULES: Rule[] = [
  { prefix: "/dashboard", open: true }, { prefix: "/notifications", open: true }, { prefix: "/learn", open: true },
  { prefix: "/super-admin", ownerOnly: true },
  { prefix: "/pos/command-center", any: ["pos.sales.view_all"] }, { prefix: "/pos/retail-command-center", any: ["pos.sales.view_all"] },
  { prefix: "/pos-sales", any: ["pos.retail.access"] }, { prefix: "/pos-workers", any: ["users.manage"] },
  { prefix: "/pos", any: ["pos.retail.access"] }, { prefix: "/sifopos", any: ["pos.retail.access"] },
  { prefix: "/devices-terminals", any: ["settings.manage"] }, { prefix: "/printing-settings", any: ["settings.manage"] },
  { prefix: "/restaurant/menu", any: ["products.manage"] }, { prefix: "/restaurant/combos", any: ["products.manage"] },
  { prefix: "/restaurant/settings", any: ["settings.manage"] }, { prefix: "/restaurant/loyalty", any: ["settings.manage", "pos.sales.view_all"] },
  ...
  { prefix: "/reports/sales-by-customer", any: ["reports.view", "financial_reports.view"] }, { prefix: "/reports", any: ["financial_reports.view"] },
  { prefix: "/payroll", any: ["hr.manage"] }, { prefix: "/employees", any: ["hr.manage"] }, { prefix: "/attendance", any: ["hr.manage"] },
  { prefix: "/leave", any: ["hr.manage"] }, { prefix: "/time-entries", any: ["hr.manage"] }, { prefix: "/timesheet", any: ["hr.manage"] },
  { prefix: "/roles", any: ["roles.manage", "users.manage"] }, { prefix: "/admin", any: ["users.manage"] }, { prefix: "/setup", any: ["settings.manage"] },
  { prefix: "/modules", any: ["settings.manage"] }, { prefix: "/industry", any: ["settings.manage"] }, { prefix: "/subscription", any: ["settings.manage"] },
  { prefix: "/onboarding", any: ["settings.manage"] }, { prefix: "/launch", any: ["settings.manage"] }, { prefix: "/system-health", any: ["settings.manage"] },
  { prefix: "/audit-logs", any: ["settings.manage", "accounting.view"] }, { prefix: "/approvals", any: ["accounting.view", "pos.sales.view_all"] },
];
const DEFAULT_RULE: Rule = { prefix: "", any: ["accounting.view"] };
export function ruleFor(path: string): Rule { const p = path.replace(/\/+$/, "") || "/"; return ROUTE_RULES.find((r) => p === r.prefix || p.startsWith(r.prefix + "/")) ?? DEFAULT_RULE; }
export function canAccessPath(a: Access | null, path: string): boolean { if (!a) return false; if (a.is_owner || a.is_super_admin) return true; const r = ruleFor(path); if (r.open) return true; if (r.ownerOnly) return false; return hasAnyPerm(a, r.any ?? []); }
export function landingFor(a: Access | null): string { if (!a || a.is_owner || a.is_super_admin) return "/dashboard"; if (hasPerm(a, "pos.retail.access")) return "/pos"; if (hasPerm(a, "pos.restaurant.access")) return "/restaurant/pos"; return "/dashboard"; }
...
export const STAFF_NAV: StaffNavGroup[] = [
  ...
  { label: "Inventory", items: [
    ...
    { title: "End of Day & Cash", url: "/inventory/end-of-day", iconName: "Calculator", any: ["inventory.view", "inventory.manage"] },
  ...
  { label: "Accounting", items: [
    { title: "Customers", url: "/customers", iconName: "Users", any: ["accounting.view"] }, { title: "Sales Invoices", url: "/invoices", iconName: "FileText", any: ["accounting.view"] },
    { title: "Receive Payments", url: "/receipts", iconName: "CreditCard", any: ["accounting.view"] }, { title: "Suppliers", url: "/suppliers", iconName: "Truck", any: ["accounting.view"] },
    { title: "Bills", url: "/bills", iconName: "FileBox", any: ["accounting.view"] }, { title: "Expenses", url: "/expenses", iconName: "Receipt", any: ["accounting.view"] },
    { title: "Banking", url: "/banking", iconName: "Landmark", any: ["accounting.view"] }, { title: "Reconciliation", url: "/reconciliation", iconName: "Scale", any: ["accounting.view"] },
    { title: "Journal Entries", url: "/journal-entries", iconName: "BookText", any: ["accounting.view"] }, { title: "Chart of Accounts", url: "/chart-of-accounts", iconName: "ListTree", any: ["accounting.view"] },
  ] },
  { label: "Reports", items: [{ title: "Reports Hub", url: "/reports", iconName: "BarChart3", any: ["financial_reports.view"] }] },
...
export function staffNav(a: Access | null): StaffNavGroup[] { if (!a) return []; return STAFF_NAV.filter((g) => !g.requires || hasAnyPerm(a, g.requires)).map((g) => ({ label: g.label, items: g.items.filter((i) => hasAnyPerm(a, i.any)) })).filter((g) => g.items.length > 0); }
export const MODULE_PERMS: Record<string, PermissionKey[]> = {
  core_home: [], sales: ["accounting.view"], purchases: ["accounting.view"], finance: ["accounting.view"], fixed_assets: ["accounting.view"], budgets: ["accounting.view"], multi_currency: ["accounting.view"],
  inventory: ["inventory.view"], hr_payroll: ["hr.manage"], crm: ["accounting.view"], projects: ["accounting.view"], reports: ["financial_reports.view"], compliance: ["accounting.view"], loans: ["accounting.view"],
  donors: ["accounting.view"], school_erp: ["accounting.view"], retail_pos: ["pos.retail.access"], restaurant: ["pos.restaurant.access"], learning: [], admin: ["users.manage", "roles.manage", "settings.manage"], platform: ["settings.manage"],
};