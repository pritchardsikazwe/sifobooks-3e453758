...
export async function recordSchoolAttendance(input: { companyId?: string; userId: string; studentId: string; attendanceDate: string; status: string; notes?: string }) {
  const companyId = requireCompanyId(input.companyId);
  const { data: existing } = await db.from("school_attendance").select("id").eq("student_id", input.studentId).eq("attendance_date", input.attendanceDate).maybeSingle();
  ...
  if (existing) {
    const { data, error } = await db.from("school_attendance").update(payload).eq("id", existing.id).select().single();
    if (error) throw error;
    return data;
  }
  const { data, error } = await db.from("school_attendance").insert(payload).select().single();
  if (error) throw error;
  return data;
}
...
export async function saveSchoolMarks(input: { companyId?: string; userId: string; assessmentId: string; studentId: string; marks: number; grade?: string; remarks?: string }) {
  ...
  const { data, error } = await db.from("school_marks").upsert({ company_id: companyId, user_id: input.userId, assessment_id: input.assessmentId, student_id: input.studentId, marks: input.marks, grade: input.grade ?? null, remarks: input.remarks ?? null }, { onConflict: "assessment_id,student_id" }).select().single();
  if (error) throw error;
  return data;
}
...
export async function updateSchoolFeeAccount(input: { companyId?: string; userId: string; studentId: string; academicYear: string; term?: string; charges: number; paid: number }) {
  ...
  const { data, error } = await db.from("school_fee_accounts").upsert({ company_id: companyId, user_id: input.userId, student_id: input.studentId, academic_year: input.academicYear, term: input.term ?? null, charges: input.charges, paid: input.paid, balance }, { onConflict: "student_id,academic_year,term" }).select().single();
  if (error) throw error;
  return data;
}
