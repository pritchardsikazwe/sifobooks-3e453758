...
export const rpcFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const auth = token ? await verifyToken(token) : null;
    if (!auth) return { data: null, error: { message: "NOT_AUTHENTICATED" } };
    return isCloudDatabaseConfigured() ? await executeCloudRpc(data.name, { ...(data.args || {}), _uid: auth.userId }) : executeRpc(data.name, { ...(data.args || {}), _uid: auth.userId });
  });
...
function executeRestaurantCheckout(args: Record<string, any>) {
  ...
  const transaction = db.transaction(() => {
    ...
    if (existingOrder) {
      ...
      db.prepare("DELETE FROM restaurant_order_items WHERE order_id=? AND user_id=?").run(existingOrder.id, uid);
      db.prepare("DELETE FROM restaurant_payments WHERE order_id=? AND user_id=?").run(existingOrder.id, uid);
