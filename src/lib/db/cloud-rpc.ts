...
export async function executeCloudRpc(name: string, args: Record<string, any>) {
  ...
  try {
    switch (name) {
      case "current_tenant": {
        const rows = await db.unsafe("SELECT company_id FROM company_members WHERE user_id=$1 ORDER BY updated_at NULLS LAST LIMIT 1", [uid]);
        return { data: rows[0]?.company_id ?? null, error: null };
      }
      case "has_role": {
        const rows = await db.unsafe("SELECT 1 FROM user_roles WHERE user_id=$1 AND role=$2 LIMIT 1", [String(args._user_id || uid), String(args._role || "")]);
        return { data: rows.length > 0, error: null };
      }
      case "can_manage_company":
      case "is_company_admin": {
        const rows = await db.unsafe("SELECT role FROM company_members WHERE user_id=$1 AND company_id=$2 AND role IN ('admin','owner') LIMIT 1", [String(args._user_id || uid), String(args._company_id || "")]);
        return { data: rows.length > 0, error: null };
      }
      case "my_access": {
        const owned = await db.unsafe("SELECT id,name FROM companies WHERE user_id=$1 LIMIT 1", [uid]);
        ...
        const member = await db.unsafe("SELECT company_id,role FROM company_members WHERE user_id=$1 ORDER BY updated_at NULLS LAST LIMIT 1", [uid]);
      ...
      case "ensure_account": {
        ...
        const rows=await db.unsafe("SELECT id FROM chart_of_accounts WHERE user_id=$1 AND account_code=$2 LIMIT 1", [uid,code]);
        if(rows[0]) return {data:rows[0].id,error:null};
        const id=crypto.randomUUID();
        await db.unsafe("INSERT INTO chart_of_accounts(id,user_id,account_code,account_name,account_type,is_active) VALUES($1,$2,$3,$4,$5,true)", [id,uid,code,String(args._name||""),String(args._type||"expense")]);
