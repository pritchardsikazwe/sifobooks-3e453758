 ...
 * Creates a bank_transactions row plus a balanced journal entry with
 * one line per allocation split.
 */
export async function postSpendMoney(opts: {
  userId: string;
  bankAccountId: string;         // bank_accounts.id
  txnDate: string;
  amount: number;                // positive = spend, negative = receive
  supplierName?: string;
  paymentMethod?: string;
  reference: string;
  memo?: string;
  allocations: Array<{
    accountId: string;           // chart_of_accounts.id
    amount: number;
    side: "DR" | "CR";
    description?: string;
  }>;
  ...
  // 1. bank_transactions row (negative for spend)
