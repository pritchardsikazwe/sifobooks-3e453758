...
export const PRICE_LEVELS: { key: PriceLevel; label: string; factor: number }[] = [
  ...
  { key: "wholesale", label: "Wholesale", factor: 0.85 }, { key: "vip", label: "VIP", factor: 0.9 },
...
export async function loadCustomers():Promise<PosCustomer[]> { try { const {data}=await supabase.from("customers").select("id,name,phone,customer_code").order("name").limit(1000); const rows=(data??[]).map((c:any)=>({id:c.id,name:c.name,phone:c.phone??null,code:c.customer_code??null})) as PosCustomer[]; void cacheRows("customers",rows); return rows; } catch { return await readCached<PosCustomer>("customers"); } }
...
export async function completeSale(draft:SaleDraft,payments:SalePayment[],changeDue:number){
  const validation=validatePosTransaction({
    lines:draft.lines.map((l)=>({itemId:l.item_id,qty:l.qty,price:l.price,unitCost:l.unit_cost,discountPct:l.discount_pct})),
    saleDiscountPct:draft.saleDiscountPct,
    taxRate:draft.taxRate ?? 16,
    taxInclusive:draft.taxInclusive ?? true,
    ...
    payments,
    changeDue,
    allowNegativeStock:draft.allowNegativeStock ?? false,
  });
...
export async function voidSale(saleId:string,reason:string){const {data,error}=await supabase.rpc("reverse_pos_sale" as any,{_sale_id:saleId,_action:"void",_reason:reason,_refund_method:null} as any);if(error)throw new Error(error.message);return data as string;}
export async function refundSale(saleId:string,refundMethod="cash",reason="Customer refund"){const {data,error}=await supabase.rpc("reverse_pos_sale" as any,{_sale_id:saleId,_action:"refund",_reason:reason,_refund_method:refundMethod} as any);if(error)throw new Error(error.message);return data as string;}
