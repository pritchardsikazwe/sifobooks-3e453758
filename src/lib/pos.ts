 ...
 * Reuses the existing catalogue, customers and accounting engine.
 */
...
async function clearStore(store: "pos_transactions") { await withStore(store, "readwrite", (os) => os.clear()); }
...
export const PRICE_LEVELS: { key: PriceLevel; label: string; factor: number }[] = [
  { key: "normal", label: "Normal", factor: 1 }, { key: "retail", label: "Retail", factor: 1 },
  { key: "wholesale", label: "Wholesale", factor: 0.85 }, { key: "vip", label: "VIP", factor: 0.9 }, { key: "customer", label: "Customer", factor: 0.95 },
];
...
export async function ensureRegister(): Promise<PosRegister | null> {
  const { data } = await supabase.from("pos_registers").select("id,name,branch,location_id").eq("is_active", true).order("name").limit(1);
  if (data && data.length) return data[0] as PosRegister;
  const { data: location } = await supabase.from("inventory_locations").select("id,name,code").eq("is_active", true).or("code.eq.CHIBOMBO,name.eq.Chibombo Store").limit(1).maybeSingle();
  const { data: created } = await supabase.from("pos_registers").insert({ name: "Register 01", branch: location?.name ?? "Main", location_id: location?.id ?? null } as any).select("id,name,branch,location_id").maybeSingle();
...
export async function loadProducts(locationId?: string | null): Promise<PosProduct[]> {
  try {
    const effectiveLocation = locationId ?? (await ensureRegister())?.location_id ?? null;
    if (effectiveLocation) {
      const { data, error } = await supabase.rpc("pos_location_stock" as any, { _location: effectiveLocation });
      if (error) throw error;
      const rows = (data ?? []).map((r: any) => ({ id: r.id, name: r.name, sku: r.sku ?? null, barcode: r.barcode ?? null, category: r.category ?? null, unit: r.unit ?? null, price: n(r.sell_price), cost: n(r.cost_price), stock: n(r.quantity_on_hand), reorder_level: n(r.reorder_level), is_active: r.is_active !== false })) as PosProduct[];
      void cacheRows("products", rows); return rows;
    }
    const { data, error } = await supabase.from("stock_items").select("id,name,sku,barcode,category,unit,sell_price,cost_price,quantity_on_hand,reorder_level,is_active").order("name").limit(2000);
    if (error) throw error;
    const rows = (data ?? []).map((r: any) => ({ id: r.id, name: r.name, sku: r.sku ?? null, barcode: r.barcode ?? null, category: r.category ?? null, unit: r.unit ?? null, price: n(r.sell_price), cost: n(r.cost_price), stock: n(r.quantity_on_hand), reorder_level: n(r.reorder_level), is_active: r.is_active !== false })) as PosProduct[];
    void cacheRows("products", rows); return rows;
...
export async function loadCustomers(): Promise<PosCustomer[]> {
  try { const { data } = await supabase.from("customers").select("id,name,phone,customer_code").order("name").limit(1000); const rows = (data ?? []).map((c: any) => ({ id: c.id, name: c.name, phone: c.phone ?? null, code: c.customer_code ?? null })) as PosCustomer[]; void cacheRows("customers", rows); return rows; } catch { return await readCached<PosCustomer>("customers"); }
}
...
export async function currentShift(registerId?: string | null) { let query = supabase.from("pos_shifts").select("*").eq("status", "open").order("opened_at", { ascending: false }).limit(1); if (registerId) query = query.eq("register_id", registerId); const { data } = await query; return ((data ?? [])[0] as any) ?? null; }
export async function openShift(registerId: string | null, cashier: string, float_: number) { if (!registerId) throw new Error("A POS register is required before opening a shift"); const { data, error } = await supabase.from("pos_shifts").insert({ register_id: registerId, cashier_name: cashier, opening_float: float_ } as any).select("*").maybeSingle(); if (error) throw new Error(error.message); return data as any; }
export async function shiftSummary(shiftId: string) { const { data: sales } = await supabase.from("pos_sales").select("id,total,status").eq("shift_id", shiftId); const completed = (sales ?? []).filter((s: any) => s.status === "completed"); const voided = (sales ?? []).filter((s: any) => s.status === "voided"); const refunds = (sales ?? []).filter((s: any) => s.status === "refunded"); const ids = completed.map((s: any) => s.id); let byMethod: Record<string, number> = {}; if (ids.length) { const { data: pays } = await supabase.from("pos_payments").select("method,amount").in("sale_id", ids); (pays ?? []).forEach((p: any) => { byMethod[p.method] = (byMethod[p.method] ?? 0) + n(p.amount); }); } return { transactions: completed.length, salesTotal: completed.reduce((a: number, s: any) => a + n(s.total), 0), voids: voided.length, refunds: refunds.length, byMethod }; }
export async function closeShift(shiftId: string, actualCash: number, expectedCash: number) { const { error } = await supabase.from("pos_shifts").update({ status: "closed", closed_at: new Date().toISOString(), actual_cash: actualCash, expected_cash: expectedCash, variance: actualCash - expectedCash } as any).eq("id", shiftId); if (error) throw new Error(error.message); }
...
export type SaleDraft = { lines: CartLine[]; totals: PosTotals; customer: PosCustomer | null; customerName: string; priceLevel: PriceLevel; saleDiscountPct: number; note?: string; shiftId?: string | null; registerId?: string | null };
...
async function adjustCachedStock(lines: CartLine[]) { try { const products = await readCached<PosProduct>("products"); const by = new Map(products.map((p) => [p.id, p])); for (const l of lines) { const p = l.item_id ? by.get(l.item_id) : null; if (p) p.stock = round2(n(p.stock) - n(l.qty)); } await cacheRows("products", products); } catch {} }
export async function listOfflineSales() { const rows = await readCached<any>("pos_transactions"); return rows.filter((r) => r.__offline); }
export async function pruneSyncedOfflineSales() { try { const q = await listQueue(); const pendingRefs = new Set(q.map((i) => i.clientId)); const rows = await readCached<any>("pos_transactions"); const keep = rows.filter((r) => !r.__offline || pendingRefs.has(r.client_ref)); if (keep.length !== rows.length) { await clearStore("pos_transactions"); if (keep.length) await cacheRows("pos_transactions", keep); } } catch {} }
export async function holdSale(draft: SaleDraft) { const sale_no = saleNumber(); const { data: sale, error } = await supabase.from("pos_sales").insert({ sale_no, client_ref: newClientId("poshold"), status: "held", customer_id: draft.customer?.id ?? null, customer_name: draft.customerName, price_level: draft.priceLevel, subtotal: draft.totals.subtotal, discount: round2(draft.totals.lineDiscount + draft.totals.saleDiscount), tax: draft.totals.tax, total: draft.totals.total, shift_id: draft.shiftId ?? null, register_id: draft.registerId ?? null } as any).select("id").maybeSingle(); if (error || !sale) throw new Error(error?.message ?? "Could not hold this sale"); if (draft.lines.length) { await supabase.from("pos_sale_items").insert(draft.lines.map((l) => ({ sale_id: (sale as any).id, item_id: l.item_id, name: l.name, sku: l.sku, qty: l.qty, price: l.price, unit_cost: l.unit_cost, discount: l.discount_pct, line_total: round2(l.qty * l.price), note: l.note ?? null })) as any); } return sale_no; }
export async function listHeldSales() { const { data } = await supabase.from("pos_sales").select("id,sale_no,customer_name,total,sold_at").eq("status", "held").order("sold_at", { ascending: false }).limit(50); return (data ?? []) as any[]; }
...
export async function voidSale(saleId: string, reason: string) { const { error } = await supabase.from("pos_sales").update({ status: "voided", void_reason: reason } as any).eq("id", saleId); if (error) throw new Error(error.message); }
export async function refundSale(saleId: string) { const { data: orig } = await supabase.from("pos_sales").select("*").eq("id", saleId).maybeSingle(); if (!orig) throw new Error("Sale not found"); if ((orig as any).status !== "completed") throw new Error("Only a completed sale can be refunded"); const { data: items } = await supabase.from("pos_sale_items").select("*").eq("sale_id", saleId); const { data: credit, error: creditError } = await supabase.from("pos_sales").insert({ sale_no: `RF-${(orig as any).sale_no ?? ""}`, client_ref: newClientId("posrefund"), status: "draft", refund_of: saleId, customer_id: (orig as any).customer_id, customer_name: (orig as any).customer_name, subtotal: -n((orig as any).subtotal), discount: -n((orig as any).discount), tax: -n((orig as any).tax), total: -n((orig as any).total), shift_id: (orig as any).shift_id ?? null, register_id: (orig as any).register_id ?? null } as any).select("id").maybeSingle(); if (creditError || !credit) throw new Error(creditError?.message ?? "Refund failed"); if (items?.length) await supabase.from("pos_sale_items").insert(items.map((r: any) => ({ sale_id: (credit as any).id, item_id: r.item_id, name: r.name, sku: r.sku, qty: -n(r.qty), price: n(r.price), unit_cost: n(r.unit_cost), line_total: -n(r.line_total) })) as any); await supabase.from("pos_payments").insert({ sale_id: (credit as any).id, method: "cash", amount: -n((orig as any).total), reference: "Refund" } as any); const { error: postError } = await supabase.rpc("complete_pos_sale", { _sale_id: (credit as any).id } as any); if (postError) throw new Error(postError.message); await supabase.from("pos_sales").update({ status: "refunded" } as any).eq("id", saleId); return (credit as any).id as string; }
