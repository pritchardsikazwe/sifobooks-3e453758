    ...
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b px-4 sm:px-6 py-3 flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[220px] max-w-xl">
          ...
          <Input placeholder="Search for Quotes, customers etc" value={q} onChange={e => setQ(e.target.value)} className="pl-9 bg-slate-50 border-slate-200" />
        </div>
        <div className="flex items-center gap-2 ml-auto">
          <QuickAddCustomer trigger={<Button variant="outline" className="gap-2"><UserPlus className="h-4 w-4" /> New Customer</Button>} onCreated={() => {}} />
      ...
      <div className="p-4 sm:p-6 space-y-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <KPI label="Accepted" value={stats.accepted} />
          <KPI label="Converted" value={stats.converted} />
        </div>

        <div className="bg-white rounded-lg border">
          <div className="flex items-center justify-between border-b px-4 flex-wrap gap-2">
            <div className="flex gap-1">
              {(["all", "draft", "sent", "accepted", "converted"] as const).map(t => (
          ...
          <div className="p-4">
            <div className="text-sm font-medium mb-3">Quotes</div>
            {loading ? (
              ...
              <div className="py-14 flex flex-col items-center text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1 max-w-xs">Create your first quote to start winning deals.</p>
                ...
                <table className="w-full text-sm">
                  <thead className="text-xs text-muted-foreground border-b">
                    <tr>
                      ...
                      <th className="text-left font-medium py-2 px-2">Valid Until</th>
                  ...
                  <tbody>
                    {filtered.map(q => (
                      <tr key={q.id} className="border-b last:border-0 hover:bg-slate-50">
                        ...
                        <td className="py-2 px-2">{q.customers?.name ?? "—"}</td>
                        <td className="py-2 px-2 text-xs">{q.issue_date}</td>
                        <td className="py-2 px-2 text-xs">{q.valid_until ?? "—"}</td>
                        <td className="py-2 px-2 text-right font-medium">{fmtMoney(q.total, q.currency)}</td>
                        <td className="py-2 px-2">
                          <Select value={q.status} onValueChange={v => updateStatus(q.id, v)} disabled={q.status === "converted"}>
                            <SelectTrigger className="h-7 w-28 text-xs"><SelectValue /></SelectTrigger>
                            <SelectContent>
                              {["draft", "sent", "accepted", "declined", "converted"].map(s => (
                                <SelectItem key={s} value={s} disabled={s === "converted"}>{s}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        </td>
                        <td className="py-2 px-2 text-right">
                          {q.status === "converted" ? (
                            ...
                            <Button size="sm" variant="outline" onClick={() => convert(q)} disabled={q.status === "declined"} className="mr-1">
