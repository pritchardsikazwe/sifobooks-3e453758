...
function DemoData(){
 ...
 const check=async()=>{const {data:u}=await supabase.auth.getUser();if(!u.user)return;const {data}=await supabase.from("companies").select("id").eq("user_id",u.user.id).eq("name","LUANSOBE SECONDARY SCHOOL").maybeSingle();setInstalled(!!data)};useEffect(()=>{void check()},[]);
 const install=async()=>{setInstalling(true);setMessage("");try{
  ...
  const existing=await supabase.from("companies").select("id").eq("user_id",uid).eq("name","LUANSOBE SECONDARY SCHOOL").maybeSingle();
  ...
  for(const [table,row] of [
   ["companies",{id:companyId,user_id:uid,name:"LUANSOBE SECONDARY SCHOOL",trading_name:"Luansobe Secondary School",address:"P O BOX 40463 LUASNSOBE SECONDARY SCHOOL MUFULIRA COPPERBELT",city:"Mufulira",country:"Zambia",base_currency:"ZMW",timezone:"Africa/Lusaka",industry:"School / Education",workspace_mode:"accounting",status:"active",is_primary:0}],
   ["company_members",{id:crypto.randomUUID(),company_id:companyId,user_id:uid,role:"owner"}],
   ["bank_accounts",{id:bankId,user_id:uid,company_id:companyId,name:"CASA — Luansobe Secondary School",bank_name:"CASA",account_number:"5786225300117",currency:"ZMW",opening_balance:48594.46,opening_date:"2026-07-01",cashbook_type:"main",is_active:1,notes:"Source account. Statements are source evidence; no automatic GL/VAT posting."}],
  ...
  const rows=all.map((r,i)=>({id:crypto.randomUUID(),user_id:uid,txn_date:r.date,description:r.description,amount:r.direction==="credit"?r.amount:-r.amount,balance:r.balance,reference:r.reference,category:"REAL_SOURCE_PENDING",source_file:r.period==="2026-07"?"Sifobooks_July_2026_Bank_Reconciliation_Working_Paper.xlsx":"CASA_Statement_August_2026.pdf",reconciled:0,currency:"ZMW",exchange_rate:1,allocated_amount:0,bank_account_id:bankId,payee:r.payee,is_allocated:0,voucher_no:"REAL-"+r.period+"-"+String(i+1).padStart(3,"0")}));
 ...
 <Card className="p-5"><div className="flex items-start gap-3"><ShieldAlert className="mt-0.5 h-5 w-5 text-amber-600"/><div className="flex-1"><h2 className="font-semibold">Source-controlled real data</h2><p className="mt-1 text-sm text-muted-foreground">Bank descriptions, references, payees and running balances are preserved. GL account, VAT treatment, allocation and final reconciliation are intentionally not posted automatically.</p><div className="mt-3 flex flex-wrap gap-2"><Badge variant="outline">July: K195,357.62 closing</Badge><Badge variant="outline">August: K15,897.62 closing</Badge><Badge variant="outline">August: 27 debits / K179,460</Badge></div></div></div><Button className="mt-5" onClick={()=>void install()} disabled={installing||installed}>{installing?"Installing…":installed?<><CheckCircle2 className="mr-2 h-4 w-4"/>Already installed</>:<>Load Company & Statements</>}</Button>{message&&<p className="mt-3 text-sm text-muted-foreground">{message}</p>}</Card>
 <Card className="p-5"><h2 className="font-semibold">What this source-data workflow tests</h2><div className="mt-3 grid gap-2 md:grid-cols-2">{["Statement import and duplicate detection","Bank → Cashbook → Reconciliation workflow","Running-balance verification","Unallocated transaction queue","Supplier/payee matching","Control exceptions and audit trail","July reconciliation with source exception","August reconciliation pending cashbook/GL comparison"].map(x=><div key={x} className="rounded-lg border p-3 text-sm">{x}</div>)}</div></Card>
 </SifoWorkspaceShell>;
}