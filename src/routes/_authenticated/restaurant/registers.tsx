...
function RestaurantRegisters(){
 ...
 const load=async()=>{const {data:u}=await supabase.auth.getUser(); if(!u.user)return; const {data,error}=await supabase.from("pos_registers").select("id,name,branch,location_id,is_active").eq("is_active",true).order("name"); if(error)toast.error(error.message); else setRows(data??[])};
 useEffect(()=>{void load()},[]);
