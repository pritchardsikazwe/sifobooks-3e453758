...
import { brandAddressLines, brandDisplayName } from "@/lib/branding";
...
export function SifoDocumentLayout({ title, number, status, description, back, actions, party, metadata, lines, lineHeaders, lineActions, totals, footer, impact }: {
  ...
  const print = () => {
    const article = document.querySelector(".sifo-print-document");
    void printHtmlDocument(`${title}${number ? ` ${number}` : ""}`, article?.innerHTML ?? "");
  };
  return (
    <div className="sifo-document-page min-h-full bg-muted/20 px-4 py-5 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <article className="sifo-print-document rounded-xl border bg-card p-5 shadow-sm sm:p-7 print:rounded-none print:border-0 print:bg-white print:p-0 print:shadow-none">
          <header className="mb-6 hidden border-b-2 border-foreground/20 pb-4 print:block"><div className="flex items-start justify-between gap-6"><div className="flex items-start gap-3">{branding.logoUrl?.startsWith("http") && <img src={branding.logoUrl} alt="" className="h-12 w-auto object-contain" />}<div><div className="text-xl font-bold">{brandDisplayName(branding)}</div><div className="mt-1 space-y-0.5 text-[10px] text-muted-foreground">{brandAddressLines(branding).slice(0, 4).map(l => <div key={l}>{l}</div>)}</div></div></div><div className="text-right"><div className="text-lg font-bold uppercase tracking-wide">{title}</div>{number && <div className="mt-1 font-mono text-xs">{number}</div>}{status && <div className="mt-1 text-xs uppercase">{status}</div>}</div></div></header>
          ...
          <footer className="mt-8 hidden border-t border-foreground/20 pt-3 print:block"><div className="flex justify-between gap-4 text-[10px] text-muted-foreground"><span>{brandDisplayName(branding)}{branding.footerNote ? ` · ${branding.footerNote}` : ""} · {new Date().toLocaleString()}</span><span>Page <span className="sifo-page-number" /></span></div></footer>
        </article>
      </div>
