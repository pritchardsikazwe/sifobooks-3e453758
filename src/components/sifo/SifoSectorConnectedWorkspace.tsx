...
export function SifoSectorConnectedWorkspace({ kind, screen }: { kind: "hotel" | "school"; screen: string }) {
  ...
  if (!userId || !companyId || rows.length === 0) return <SifoIndustryWorkspace kind={kind} screen={screen} />;

  if (screen === "/hotel/reservations") return <HotelReservationEditor busy={busy} onSave={(v) => action(() => createHotelReservation({ ...v, userId, companyId }))} rows={rows} />;
  if (screen === "/school/attendance") return <SchoolAttendanceEditor busy={busy} onSave={(v) => action(() => recordSchoolAttendance({ ...v, userId, companyId }))} rows={rows} />;
  if (screen === "/school/exams") return <SchoolMarksEditor busy={busy} onSave={(v) => action(() => saveSchoolMarks({ ...v, userId, companyId }))} rows={rows} />;
  if (screen === "/school/fees") return <SchoolFeeEditor busy={busy} onSave={(v) => action(() => updateSchoolFeeAccount({ ...v, userId, companyId }))} rows={rows} />;
  return <SifoIndustryWorkspace kind={kind} screen={screen} />;
}

function Shell({ title, children }: { title: string; children: React.ReactNode }) { return <div className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl space-y-5"><Card><CardHeader><CardTitle>{title}</CardTitle></CardHeader><CardContent>{children}</CardContent></Card></div></div>; }
