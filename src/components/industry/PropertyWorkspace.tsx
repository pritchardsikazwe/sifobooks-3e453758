...
export function PropertyWorkspace(){
 ...
 const [charges,setCharges]=useState<Charge[]>([]); const [payments,setPayments]=useState<Payment[]>([]);
 ...
 const load=async()=>{
  ...
  setProperties((a.data??[]) as any); setUnits((b.data??[]) as any); setTenants((c.data??[]) as any); setLeases((d.data??[]) as any); setCharges((e.data??[]) as any); setPayments((f.data??[]) as any); setLoading(false);
 };
 useEffect(()=>{void load()},[]);
 ...
 return <RequireModule moduleKey="property_management"><div className="space-y-5">
  ...
  <Tabs value={tab} onValueChange={setTab}><TabsList className="grid w-full grid-cols-3 lg:grid-cols-7"><TabsTrigger value="dashboard">Dashboard</TabsTrigger><TabsTrigger value="properties">Properties</TabsTrigger><TabsTrigger value="units">Units</TabsTrigger><TabsTrigger value="tenants">Tenants</TabsTrigger><TabsTrigger value="leases">Leases</TabsTrigger><TabsTrigger value="collections">Collections</TabsTrigger><TabsTrigger value="bnb">Daily / BnB</TabsTrigger></TabsList>
...
function Editor({kind,form,setForm,properties,units,tenants,onClose,onSave}:any){
 ...
 if(kind==="charge")return common(<><SelectField l="Lease" v={form.lease_id} set={v=>set("lease_id",v)} opts={[]} /><Field l="Description" v={form.description} set={v=>set("description",v)}/><Field l="Amount" v={form.amount} set={v=>set("amount",Number(v)||0)}/><Field l="Due date" v={form.due_date} set={v=>set("due_date",v)} type="date"/></>);
